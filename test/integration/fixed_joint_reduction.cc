...
const std::string SDF_TEST_FILE_EXTENSION =
  std::string(PROJECT_SOURCE_PATH)
  + "/test/integration/fixed_joint_reduction_extension.urdf";
const std::string SDF_TEST_FILE_EXTENSION_SDF =
  std::string(PROJECT_SOURCE_PATH)
  + "/test/integration/fixed_joint_reduction_extension.sdf";
const std::string SDF_TEST_FILE_EXTENSION_EMPTY_ROOT =
  std::string(PROJECT_SOURCE_PATH)
  + "/test/integration/fixed_joint_reduction_empty_root.urdf";
const std::string SDF_TEST_FILE_EXTENSION_EMPTY_ROOT_SDF =
  std::string(PROJECT_SOURCE_PATH)
  + "/test/integration/fixed_joint_reduction_empty_root.sdf";
...
TEST(SDFParser, FixedJointReductionExtensionTest)
{
  ...
  sdf::init(urdfRobot);
  ASSERT_TRUE(sdf::readFile(SDF_TEST_FILE_EXTENSION, urdfRobot));

  // load sdf
  sdf::SDFPtr sdfRobot(new sdf::SDF());
  sdf::init(sdfRobot);
  ASSERT_TRUE(sdf::readFile(SDF_TEST_FILE_EXTENSION_SDF, sdfRobot));
...
// Compare parsing results with a pre-built verified sdf
// as well as with hardcoded expected values.
TEST(SDFParser, FixedJointReductionExtensionEmptyRootTest)
{
  // load urdf file, load sdf file.
  // check to see that urdf load results are consistent with sdf load.
  ...
  sdf::SDFPtr urdfRobot(new sdf::SDF());
  sdf::init(urdfRobot);
  ASSERT_TRUE(sdf::readFile(SDF_TEST_FILE_EXTENSION_EMPTY_ROOT, urdfRobot));

  // load sdf
  sdf::SDFPtr sdfRobot(new sdf::SDF());
  sdf::init(sdfRobot);
  ASSERT_TRUE(sdf::readFile(SDF_TEST_FILE_EXTENSION_EMPTY_ROOT_SDF, sdfRobot));
